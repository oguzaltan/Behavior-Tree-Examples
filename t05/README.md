# t05 — CrossDoor (Fallback, Retry, SubTree)
...
From the repository root (or from inside `t05`):

```bash
# configure & build (out-of-source)
cmake -S t05 -B t05/build
cmake --build t05/build --parallel
```

Run

```bash
./t05/build/bt_simple_pick
```
...
```bash
cmake -S t05 -B t05/build -DCMAKE_PREFIX_PATH=/path/to/behaviortree/install
```
